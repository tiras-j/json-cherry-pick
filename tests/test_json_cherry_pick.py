...
class TestExceptions(object):
    ...
    def test_key_error(self, testdata):
        base = testdata[0]
        with pytest.raises(KeyError):
            jcp.extract(base, 'invalid_key')

    def test_tokenized_key_error(self, testdata):
        base = testdata[0]
        with pytest.raises(KeyError):
            jcp.extract(base, 'test_obj.test_empty_obj')

    def test_index_error(self, testdata):
        base = testdata[0]
        with pytest.raises(IndexError):
            jcp.pluck_list(base, 'test_arr', 5)

    def test_edge_case_index_error(self, testdata):
        base = testdata[0]
        with pytest.raises(IndexError):
            jcp.pluck_list(base, 'test_empty_arr', 0)

    def test_reverse_index_exc(self, testdata):
        base = testdata[0]
        with pytest.raises(IndexError):
